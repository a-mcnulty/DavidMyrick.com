<article class="inner gridpage ajaxload" data-bodyclass="no_divider light_nav menuOn" data-metatitle="<?= $metaTitle ?>">
            ...
            <?php for ($count = 1; $item = mysqli_fetch_array($images); ++$count) { ?>
                ...
                <a href="<?= $theLink ?>" class="cell <?= $cellClass ?>" data-id="<?= $item['id'] ?>" id=" <?= $count ?>" <?php if ($hasAutoVideo) { ?>data-autovideo='<?= $videoStr ?>' <?php } ?>>
                    ...
                    <div class="mediawrap" data-ratio=".41">
                        <img class="photo loadme <?php if ($hasAutoVideo && $itemVideo['bgVideoType'] != "view") { ?>loadmeview<?php } ?> <?= $ext ?>" src="<?= $loaderImg ?>" data-img="<?= $item['img'] ?>" data-width="<?= $item['width'] ?>" data-height="<?= $item['height'] ?>">
