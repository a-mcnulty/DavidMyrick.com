...
// Settings
	...
	//if($_SESSION['up_category'] == 'Personal') { $save_path .= "personal/"; }

 $save_path = getOption("videoPath");
file_put_contents('debug.log', 'save_path='.$save_path."\n", FILE_APPEND);
	$upload_name = "Filedata";
	$max_file_size_in_bytes = 2147483647;				// 2GB in bytes
	$extension_whitelist = array("mp4", "webm", "ogv", "ogg", "flv");	// Allowed file extensions
	$valid_chars_regex = '.A-Z0-9_ !@#$%^&()+={}\[\]\',~`-';				// Characters allowed in the file name (in a Regular Expression format)

// Other variables
file_put_contents('debug.log', 'other variables'."\n", FILE_APPEND);
...
file_put_contents('debug.log', 'validate upload'."\n", FILE_APPEND);
...
file_put_contents('debug.log', 'file extension'."\n", FILE_APPEND);
...
// Process the file
	/*
		...
		been saved.
	*/
file_put_contents('debug.log', 'process file'."\n", FILE_APPEND);
file_put_contents('debug.log', $save_path.$file_name."\n", FILE_APPEND);
	...
	if ($num == 0) {
		...
		$query = "INSERT INTO videos ($field, picid) VALUES ('$file_name', '$picID')";
	
	} else {
	
		$query = "UPDATE videos SET $field = '$file_name' WHERE picid = '$picID'";
